...
import { createProperty, deleteProperty, getAllProperties, getPropertyById, updateProperty, SearchController, getAllPropertiesByOwnerId, predictPrice } from "../controllers/propertyController";
...
import { uploadMultipleWithRelativePath } from "../middlewares/uploadMiddleware";
import { getAllLeasesByOwnerId } from "../controllers/leaseController";
 ...
 *     description: Retrieve the predicted price.
 *     responses:
 *       200:
 *         content:
 *           application/json:
 ...
 *              type: object
 */
propertyRouter.post("/predict", predictPrice);
 ...
 */
propertyRouter.post("/create", verifyAuth, verifieRole(["owner"]), uploadMultipleWithRelativePath, createProperty);
 ...
 *         description: Property not found
 */
propertyRouter.put("/:id",verifyAuth, verifieRole(["owner"]), uploadMultipleWithRelativePath,updateProperty);
 ...
 *     description: Retrieve properties owned by a specific owner.
 *     parameters:
 *       - in: path
 *         name: ownerId
 *         required: true
 ...
 *         description: Properties retrieved successfully
 */
propertyRouter.get("/owner/:ownerId", verifyAuth, verifieRole(["owner"]), getAllPropertiesByOwnerId);
