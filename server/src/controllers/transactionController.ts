...
const createTransaction = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const  payerId = decoded.userId;
        ...
        await updateUserTransactionsAsPayer(transaction.payerId,transaction._id);
        await updateUserTransactionsAsReceiver(transaction.receiverId,transaction._id);
...
const getAllTransactionsByUserId = async (req: Request, res: Response): Promise<void> => {
    try {
...
const deleteTransaction = async (req: Request, res: Response): Promise<void> => {
    try {
       ...
       await transactionRepository.delete(transactionId);
        ...
        await deleteUserTransactionAsPayer(new ObjectId(transaction!.payerId) , new ObjectId(transaction!._id));
        await deleteUserTransactionAsReceiver(new ObjectId(transaction!.receiverId),new ObjectId(transaction!._id));
...
export {getAllTransactions,getTransactionById,createTransaction,updateTransaction,deleteTransaction,getAllTransactionsByUserId}